...
class Dynamic_time_warping(object):
    ...
    def matching(self, reconJ, exer, lowpass=True):
        ...
        if not self.deflag:  # Not yet decreasing
            if np.mod(self.seqlist.shape[0]-self.presv_size-1, 10) == 0:
                ...
                if len(self.order[self.oidx]) > 1:
                    if self.seqlist.shape[0] > 1:
                        result = self.clip(self.seqlist, exer.no)
                        if result != []:
                            endidx = result[0]
                    ...
                    self.dist_p, _ = fastdtw(exer.gt_data[self.gt_idx], test_data_p, self.jweight, dist=self.wt_euclidean)
                    if (self.seqlist.shape[0] == 1+self.presv_size):  # new movement initail setting
                        self.dpfirst, _ = fastdtw(exer.gt_data[self.gt_idx], test_data_p[:2], self.jweight, dist=self.wt_euclidean)
            ...
            test_data_p = self.seqlist + np.atleast_2d((exer.gt_data[self.gt_idx][0, :] - self.seqlist[0, :]))
            self.dist_p, path_p = fastdtw(exer.gt_data[self.gt_idx], test_data_p, self.jweight, dist=self.wt_euclidean)
