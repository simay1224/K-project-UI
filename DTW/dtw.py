...
class Dtw(object):
    ...
    def seg_update(self, endidx):
        ...
        """
        if self.seglist == []:
            self.seglist.append([self.seginidx,self.seginidx+endidx])
            self.seginidx    = self.seginidx+endidx
        else:
                ...
                if (len(self.order[exeno][self.oidx])>1 ):# 
                    ...
                    if (self.seqlist.shape[0] == 1+self.presv_size): # new movement initail setting
                        ...
                        if (self.dpfirst - self.dist_p)>self.decTh:
                            ...
                            print('deflag on')
                            print('=========')
                            self.deidx       = self.seqlist_reg.shape[0]
                            self.deflag     = True
