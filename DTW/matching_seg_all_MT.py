# -*- coding: utf-8 -*-
"""
Created on Thu Sep 14 22:42:30 2017
...
#dst_path  = 'C:/Users/Dawnknight/Documents/GitHub/K_project/DTW/figure/0914/7 joints/'
...
filelist = glob.glob(os.path.join(src_path,'*.pkl'))[:9]
flen     = len(filelist)
...
def matching(Start,End):
    for infile in filelist[Start:End]:
        print infile
        print('\n')
...
for i in range(nthread):
    pool.append(threading.Thread(target=matching,args=[i*step,(i+1)*step],))

#Start all threads in thread pool
import time
ST = time.clock()
for thidx, thread in enumerate(pool):
  thread.start()
#  response = queue.get()
#  thread_results[thidx] = response
#Kill all threads
for thread in pool:
  thread.join()
print time.clock() - ST
