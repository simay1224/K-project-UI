...
class Exeinst(object):
    ...
    def __init__(self):
        ...
        #                     '\n4. Repeat 4 times.'\
        #                     '\n5. Put down your arms.'
        #because we are hiding the exercie 5, exercise 6 will be displayed as 5 now, and 7 as 6 for user interface
    ...
    # change in range because we removed one exercise
        self.title = [''.join(self.str['name'][x]) for x in range(1, 7)]
        self.title = ['Exercise '+repr(i+1)+': '+self.title[i] for i in range(len(self.title))]
        #becasue we removed one exercise, now we have 1~6 exercises
        self.title.append('(Press 1~6 to do Execise 1~6.)')
