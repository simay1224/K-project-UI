...
for cor_th in [0.5]:  # ============================================================#
    ...
    for adj_type in [0]:  # ============================================================#
        ...
        for rel_Btype in [True]:                        # ============================================================#
            ...
            Err_all_x = np.zeros((N/step))
            Err_all_y = np.zeros((N/step))
            Err_all_z = np.zeros((N/step))
            
            Err_unrel_x = np.zeros((N/step))
            Err_unrel_y = np.zeros((N/step))
            Err_unrel_z = np.zeros((N/step))
...
#                            Rdata = cPickle.load(open(Rfile,'rb'),encoding = 'latin1')[4:10,:]
                            ...
                            k_mean = np.mean(Kdata,axis = 2).reshape(6,3,-1) 
                            Kdata_mean = Kdata - k_mean
...
#                                    R[R< Rel_th] = 0
                                ...
                                mx = np.matmul(np.matmul(inv(np.matmul(W.T,W)+gamma_x*Lapmtx_x),np.matmul(W.T,W)),Kdata_mean[:,0,idx].reshape(6,-1))
                                my = np.matmul(np.matmul(inv(np.matmul(W.T,W)+gamma_y*Lapmtx_y),np.matmul(W.T,W)),Kdata_mean[:,1,idx].reshape(6,-1))
                                mz = np.matmul(np.matmul(inv(np.matmul(W.T,W)+gamma_z*Lapmtx_z),np.matmul(W.T,W)),Kdata_mean[:,2,idx].reshape(6,-1))
         
                                mx += k_mean[:,0].reshape(6,-1)
                                my += k_mean[:,1].reshape(6,-1)
                                mz += k_mean[:,2].reshape(6,-1)
...
#                                                      (Mdata[R==10**-6,2,idx]-mz[R==10**-6].flatten())**2)**0.5)
                                ...
                                unerr_x = unerr_x + sum(abs(Mdata[R<Rel_th,0,idx]-mx[R<Rel_th].flatten()))
                                unerr_y = unerr_y + sum(abs(Mdata[R<Rel_th,1,idx]-my[R<Rel_th].flatten()))
                                unerr_z = unerr_z + sum(abs(Mdata[R<Rel_th,2,idx]-mz[R<Rel_th].flatten()))
                                
#                                err    = err   + sum(((Mdata[:,0,idx]-mx.flatten())**2+\
#                                                      (Mdata[:,1,idx]-my.flatten())**2+\
#                                                      (Mdata[:,2,idx]-mz.flatten())**2)**0.5)
                                
                                err_x  = err_x + sum(abs(Mdata[:,0,idx]-mx.flatten()))
                                err_y  = err_y + sum(abs(Mdata[:,1,idx]-my.flatten()))
                                err_z  = err_z + sum(abs(Mdata[:,2,idx]-mz.flatten()))

                          
                        Err_x = err_x/cnt
                        Err_y = err_y/cnt
                        Err_z = err_z/cnt
                        unErr_x = unerr_x/uncnt
                        unErr_y = unerr_y/uncnt
                        unErr_z = unerr_z/uncnt
        
                        Err_all_x[ii//step]   = Err_x
                        Err_all_y[ii//step]   = Err_y
                        Err_all_z[ii//step]   = Err_z
                        Err_unrel_x[ii//step] = unErr_x
                        Err_unrel_y[ii//step] = unErr_y
                        Err_unrel_z[ii//step] = unErr_z
                
            fname ='./data/GSP/diff/Err_'+titlename+'_b_mean.h5'
            f = h5py.File(fname,'w')
            f.create_dataset('all_x',data = Err_all_x)
            f.create_dataset('all_y',data = Err_all_y)
            f.create_dataset('all_z',data = Err_all_z)
            f.create_dataset('unrel_x',data = Err_unrel_x)
            f.create_dataset('unrel_y',data = Err_unrel_y)
            f.create_dataset('unrel_z',data = Err_unrel_z)
